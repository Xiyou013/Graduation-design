    ...
    <div class="nadd_con">
      ...
      <div class="nadd_cn_label">
        ...
        <div class="nadd_cnl_con" v-for="(item, index) in nlabel" :key="index">
          <span class="newLable">{{ item.ncontent }}</span>
        </div>
...
export default {
  ...
  methods: {
    ...
    async refresh() {
      ...
      let res = await this.$axios.get(`/api/admin/newsdetails/${this.newsId}`);
      console.log(res.data);
      if (res.data) {
        const {
          kjwl_n_content,
        ...
        this.selectCascader();
        this.nlabel = JSON.parse(qyss_n_labelPath);
        this.array = JSON.parse(qyss_n_labelName);
        // ldata.forEach((item) => {
        //   console.log(item);
        //   this.nlabel.push({ ncontent: item.kjwl_nl_name });
        // });
        console.log("label:", this.nlabel, this.array);
    ...
    async newInfSave() {
      ...
      if (
        ...
        var res = await this.$axios.post("/api/admin/newseditor", {
