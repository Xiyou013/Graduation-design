<template>
  <div class="news-opinion" v-loading="loading">
    ...
    <div class="news-opinion__block" v-show="showBlock">
      <div class="news-list" v-for="(item, index) in dataList" :key="index" @click="onNews(item)">
        <div class="list-info">
          ...
          <div class="info-source-time">
            <div class="info-source">
              <span style="color: #bfbfbf">来源: </span>
...
export default {
  ...
  methods: {
    ...
    onNews(item) {
      ...
      this.$router.push({name:"news-index",params:{id:item.kjwl_n_id,type:variableEnum.newsKeyType.person_news,personId:this.personid}});
    },
    async getPersonCollectNews() {
      this.loading = true;
      console.log("personData:", this.personData);
      try {
        var res = await this.$axios.get(
          `/api/www/getPersonCollectNews/${this.personid}`
        );
        if (res) {
          console.log("getPersonCollectNews:", res);
